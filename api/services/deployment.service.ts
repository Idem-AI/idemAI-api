...
export class DeploymentService {
  ...
  constructor() {
    this.repository = RepositoryFactory.getRepository<DeploymentModel>(TargetModelType.DEPLOYMENT);
  }

  async generateDeployment(userId: string, projectId: string, data: Omit<DeploymentModel, 'id' | 'createdAt' | 'updatedAt' | 'projectId'>): Promise<DeploymentModel> {
    const newDeploymentData: Omit<DeploymentModel, 'id' | 'createdAt' | 'updatedAt'> = {
      projectId,
      ...data,
    };
  ...
  async getDeploymentsByProjectId(userId: string, projectId: string): Promise<DeploymentModel[]> {
    ...
    return allDeployments.filter((deployment: DeploymentModel) => deployment.projectId === projectId);
  }
  ...
  async updateDeployment(userId: string, deploymentId: string, data: Partial<Omit<DeploymentModel, 'id' | 'projectId' | 'createdAt' | 'updatedAt'>>): Promise<DeploymentModel | null> {
    const updatePayload: Partial<Omit<DeploymentModel, 'id' | 'createdAt' | 'updatedAt'>> = data;
