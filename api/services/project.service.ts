...
class ProjectService {
  ...
  constructor() {
    this.projectRepository = RepositoryFactory.getRepository<ProjectModel>('projects');
  }
  ...
  async createUserProject(
    ...
    try {
      const projectToCreate: Omit<ProjectModel, 'id' | 'createdAt' | 'updatedAt'> = {
        ...projectData,
        userId: userId,
      };

      const newProject = await this.projectRepository.create(projectToCreate, userId);
  ...
  async getUserProjectById(
    ...
    try {
      ...
      if (!project) {
        console.log(`Project ${projectId} not found for user ${userId} via repository`);
        return null;
      }
      ...
      console.error(
        `Error fetching project ${projectId} in service:`,
        error
      );
      throw error;
    }
  ...
  async deleteUserProject(userId: string, projectId: string): Promise<void> {
    ...
    try {
      ...
      if (success) {
        ...
        console.log(`Project ${projectId} not found for deletion or delete failed via repository`);
      }
    } catch (error) {
      console.error(
        `Error deleting project ${projectId} in service:`,
        error
      );
      throw error;
    }
  ...
  async editUserProject(
    ...
    try {
      const updatedProject = await this.projectRepository.update(projectId, updatedData, userId);
      if (updatedProject) {
        console.log(`Project ${projectId} updated successfully via repository`);
      } else {
        console.log(`Project ${projectId} not found for update or update failed via repository`);
        throw new Error(`Project ${projectId} not found for update or update failed.`);
      }
    } catch (error) {
      console.error(
        `Error updating project ${projectId} in service:`,
        error
      );
      throw error;
    }
  ...
  getProjectDescriptionForPrompt(project: ProjectModel): string {
    const constraints = project.constraints && project.constraints.length > 0 ? project.constraints.join(', ') : 'Non spécifiées';
    const teamSize = project.teamSize !== undefined ? `${project.teamSize} développeurs` : 'Non spécifiée';
