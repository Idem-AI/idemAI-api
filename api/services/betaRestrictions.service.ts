...
export class BetaRestrictionsService {
  ...
  constructor() {
    ...
    this.restrictions = {
      ...
      restrictedPrompts: (process.env.BETA_RESTRICTED_PROMPTS || 'complex-branding,full-charter').split(',')
    };

    logger.info(`BetaRestrictionsService initialized - Beta mode: ${this.isBeta}`, this.restrictions);
  }
  ...
  validatePromptParams(promptType: string, params: any): FeatureValidationResult {
    ...
    if (adjustedParams.llmOptions?.maxOutputTokens) {
      ...
      logger.info(`Adjusted maxOutputTokens to ${adjustedParams.llmOptions.maxOutputTokens} for beta`);
    }
