...
/**
 * Contrôleur SSE pour la génération de concepts de logos avec streaming en temps réel
 */
export const generateLogoConceptsStreamingController = async (
  req: CustomRequest,
  res: Response
): Promise<void> => {
  const { projectId } = req.params;
  ...
  logger.info(
    `generateLogoConceptsStreamingController called - UserId: ${userId}, ProjectId: ${projectId}`
  );

  try {
    if (!userId) {
      logger.warn(
        "User not authenticated for generateLogoConceptsStreamingController"
      );
      res.status(401).json({ message: "User not authenticated" });
      return;
    }
    ...
    if (!projectId) {
      logger.warn(
        "Project ID is required for generateLogoConceptsStreamingController"
      );
      res.status(400).json({ message: "Project ID is required" });
      return;
    }
    ...
    // Fonction callback pour envoyer les événements SSE
    const sendSSEEvent = async (result: ISectionResult) => {
      const eventData = {
      ...
      logger.info(
        `SSE event sent - Step: ${result.name}, Type: ${result.type}`
      );
    };

    // Démarrer la génération avec streaming
    const result = await brandingService.generateLogoConceptsWithStreaming(
      userId,
      projectId,
      sendSSEEvent
    );
    ...
    const completionEvent = {
      type: "completed",
      stepName: "Logo Generation Process",
      data: JSON.stringify(result),
      summary: `Génération terminée avec succès: ${result.logos.length} concepts créés`,
      timestamp: new Date().toISOString(),
      parsedData: result,
    };
    ...
    logger.info(
      `SSE logo generation completed successfully - UserId: ${userId}, ProjectId: ${projectId}, LogoCount: ${result.logos.length}`
    );
  } catch (error: any) {
    logger.error(
      `Error in generateLogoConceptsStreamingController - UserId: ${userId}, ProjectId: ${projectId}: ${error.message}`,
      { stack: error.stack, body: req.body }
    );

    // Envoi de l'événement d'erreur via SSE
    const errorEvent = {
      type: "error",
      stepName: "Logo Generation Error",
      data: `Erreur lors de la génération: ${error.message}`,
