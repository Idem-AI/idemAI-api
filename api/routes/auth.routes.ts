...
authRoutes.post("/sessionLogin", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).send({ success: true, message: 'Session cookie created successfully.' });
  } catch (error: any) {
    console.error('Error creating session cookie:', error);
    res.status(401).send({ success: false, message: 'UNAUTHORIZED REQUEST!', error: error.message });
  }
});

authRoutes.get("/profile", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(401).json({ message: "Unauthenticated: Invalid or expired session.", error: error.message });
  }
});
